...
DVPE (Daisy Visual Programming Environment) provides a visual block-based interface for designing Daisy patches. This section bridges the C++ agent's expertise to the DVPE ecosystem.

### 14.1 Workflow: Mermaid as Design Buffer

Mermaid diagrams act as an **intermediate design layer** between natural language requirements and DVPE block implementations:

```
User Request → [Mermaid Signal Flow] → [DVPE Block Mapping] → C++ Code
                    ↑                       ↑
              Agent's Design         Available Blocks
              (Flexible)             (Fixed Library)
```

**Why this works**:
- Agent can draw **any** signal flow in Mermaid (unconstrained creativity)
- Then maps to **available** DVPE blocks (constrained implementation)
- Gap analysis reveals missing blocks needing custom implementation

### 14.2 Mermaid Signal Flow Template

Use this format for DSP signal flow diagrams:

> **Note**: CV/Gate I/O (4 CV + 2 Gate) are **not used by default**. Only include them if the design explicitly requires external modular integration.
...
- `[Block Type]` - Processing blocks match DVPE block names
- `K1-K8[...]` - Daisy Field knobs (always specify actual parameter)
- `MIDI[...]` - MIDI source blocks (hardware MIDI default)
- `OUT[...]` - Hardware outputs
- `CV_IN[...]` / `GATE_IN[...]` - Only if explicitly needed for modular integration

### 14.3 DVPE Block Structure

DVPE blocks are defined in `dvpe_CLD/src/core/blocks/definitions/`. Each block implements the `BlockDefinition` interface:

```typescript
interface BlockDefinition {
  id: string;              // Unique identifier (e.g., 'oscillator')
  className: string;       // DaisySP class (e.g., 'daisysp::Oscillator')
  displayName: string;     // UI name (e.g., 'OSCILLATOR')
  category: BlockCategory; // Sources, Filters, Effects, etc.
  
  // C++ Code Generation
  initMethod?: string;     // Init method (e.g., 'Init')
  initParams?: string[];   // Init params (e.g., ['sample_rate'])
  processMethod?: string;  // Process method (e.g., 'Process')
...
| Path | Description |
|------|-------------|
| `dvpe_CLD/src/core/blocks/definitions/` | Block definition TypeScript files |
| `dvpe_CLD/src/types/blocks.ts` | Core type interfaces |
| `dvpe_CLD/src/core/blocks/BlockRegistry.ts` | Block registration system |
| `_block_diagrams/` | Mermaid diagram archive |
| `_block_diagrams_code/` | Generated C++ from diagrams |

### 14.5 Mermaid → DVPE Mapping Workflow

1. **Design**: Create Mermaid diagram with desired signal flow
2. **Audit**: Compare diagram nodes to available DVPE blocks
3. **Map**: For each node, find matching `BlockDefinition.id`
4. **Gap Analysis**: Identify nodes without DVPE equivalents
5. **Implement**: Either use custom C++ or create new DVPE block

**Example Mapping**:
| Mermaid Node | DVPE Block ID | Available? |
|--------------|---------------|------------|
| `OSC[Oscillator]` | `oscillator` | ✅ |
| `FILT[Moog Filter]` | `moogladder` | ✅ |
| `ENV[ADSR]` | `adsr` | ✅ |
| `CHORUS[Chorus]` | `chorus` | ✅ |
| `CUSTOM[My Effect]` | — | ❌ (needs implementation) |

### 14.6 DVPE Block Categories

| Category | Examples | Use Case |
|----------|----------|----------|
| `Sources` | oscillator, fm2, varisaw | Sound generation |
| `Filters` | svf, moogladder, tone | Frequency shaping |
| `Effects` | chorus, flanger, phaser, decimator | Audio processing |
| `Modulators` | adsr, adenv, lfo, phasor | CV generation |
| `Dynamics` | compressor, limiter, crossfade | Level control |
| `User I/O` | knob, cv_input, audio_out | Hardware interface |
| `Utility` | vca, mixer, delay | Signal routing |
| `Drums` | hihat, analogbassdrum, syntheticsnaredrum | Percussion |
| `Physical Modeling` | stringvoice, modalvoice | Physical synthesis (LGPL) |

---

**END OF DAISY_EXPERT_AGENT v1.1**
